...
export default (props) => {
  ...
  const [City, setCity] = useState(item != null ? item.address_1 : null);

  const [House, setHouse] = useState(
    item.coordinate_1 != null ? JSON.parse(item.coordinate_1).latitude : null
  );
  const [Landmark, setLandmark] = useState(
    item.coordinate_1 != null ? JSON.parse(item.coordinate_1).longitude : null
  );
    ...
    //////footer button///// */}
        <View style={{ flex: 1 }}>
          ...
          >
            ...
            >
              {item.address_1 === null || item.coordinate_1 === null ? (
                <ThemeButton
                  title="Add Address 2"
                  buttonStyle={{
